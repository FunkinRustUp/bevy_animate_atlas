        ...
        for e in fr["E"].as_array().unwrap_or(&vec![]) {
            if let Some(asi) = e.get("ASI") {
                ...
                let child_frame = match loop_type {
                    "SF" => first_frame.min(child_total - 1),
                    "PO" => raw_ff.min(child_total - 1),
                    _ => raw_ff % child_total,
                };
...
pub fn build_part_mesh(
    ...
    let transform = |lx: f32, ly: f32| -> Vec3 {
        Vec3::new(
    ...
    let (uv_tl, uv_tr, uv_br, uv_bl) = if sprite.rotated {
        (
            Vec2::new((sprite.x + sprite.w) / sheet_w, sprite.y / sheet_h),
            Vec2::new(
                (sprite.x + sprite.w) / sheet_w,
                (sprite.y + sprite.h) / sheet_h,
            ),
            Vec2::new(sprite.x / sheet_w, (sprite.y + sprite.h) / sheet_h),
            Vec2::new(sprite.x / sheet_w, sprite.y / sheet_h),
        )
    } else {
        (
            Vec2::new(sprite.x / sheet_w, sprite.y / sheet_h),
    ...
    let positions: Vec<[f32; 3]> = vec![
        transform(0.0, 0.0).into(),
        transform(quad_w, 0.0).into(),
        transform(quad_w, quad_h).into(),
        transform(0.0, quad_h).into(),
    ];

    let uvs: Vec<[f32; 2]> = vec![uv_tl.into(), uv_tr.into(), uv_br.into(), uv_bl.into()];

    // White vertex colors
    let colors: Vec<[f32; 4]> = vec![[1.0, 1.0, 1.0, 1.0]; 4];

    // All normals face the camera (positive Z in Bevy's 2D coordinate system)
